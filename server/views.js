...
router.get('/r/:id', (req, res) => {
    ...
    db.users.find({ 'library.lists.externalId': id }, (err, users) => {
        ...
        let model = {
            ...
            renderedTotals,
            optionalFields: library.optionalFields,
            renderedDescription: md.render(list.description || ''),
...
router.get('/csv/:id', (req, res) => {
    ...
    db.users.find({ 'library.lists.externalId': id }, (err, users) => {
        ...
        let out = 'Item Name,Category,desc,qty,weight,unit,url,price,worn,consumable,photo\n';
            ...
            if (category) {
                for (const j in category.categoryItems) {
                    ...
                    if (categoryItem) {
                        ...
                        let photoUrl = '';
                        if (item.image) {
                            photoUrl = `https://i.imgur.com/${item.image}l.jpg`;
                        } else if (item.imageUrl) {
                            photoUrl = item.imageUrl;
                        }
                        itemRow.push(photoUrl);
...
const renderListTotals = function (list, totalsTemplate, unitSelectTemplate, unit) {
    ...
    out.totalWornPrice = totalWornPrice;
    out.totalWornPriceDisplay = totalWornPrice ? totalWornPrice.toFixed(2) : '';
